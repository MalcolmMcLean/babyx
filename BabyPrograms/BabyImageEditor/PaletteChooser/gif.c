...
GIF *loadanimatedgif(char *fname)
{
    ...
    do {
        int disposal;
        int transparent = loadtransparency(fp, &disposal);
        printf("trans %d disposal %d\n", transparent, disposal);
        ...
        answer->frames[answer->Nframes-1].transparent = transparent;
        answer->frames[answer->Nframes-1].disposal = disposal;
    ...
    switch(gif->frames[gif->current].disposal)
    {
        ...
        case 2:
            ...
            for(y=0;y<gif->frames[gif->current].height;y++)
                ...
                {
                    buff[(y+dy)*gif->width + x + dx] = gif->background;
                }
            break;
