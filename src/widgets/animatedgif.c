...
static int loadtransparency(FILE *fp, int *disposal);
...
static int skipapplicationcontrolblock(FILE *fp, GRAPHICSCONTROLEXTENSION *gce)
{
	int ch;
	int len;
...
static int loadtransparency2(FILE *fp, GRAPHICSCONTROLEXTENSION *gce)
{
	int len;
	int pack;
	...
	if (len < 4)
		return -1;
	pack = fgetc(fp);
	gce->hastransparency = pack & 0x01;
	gce->disposal = (pack >> 2) & 0x07;
	gce->delay = fgetu16le(fp);
	gce->transparent = fgetc(fp);
	len -= 4;
	while (len--)
		fgetc(fp);
	if (fgetc(fp) != 0)
...
static int loadgce(FILE *fp, GRAPHICSCONTROLEXTENSION *gce)
{
	int sig;
	int len;
	int res;
	...
	while (1)
	{
		...
		{
			skipapplicationcontrolblock(fp, gce);
		}
	}
...
GIF *floadanimatedgif(FILE *fp)
{
    ...
    answer = malloc(sizeof(GIF));
    if(!answer)
        goto error_exit;
    answer->Nframes = 0;
    answer->frames = 0;
    answer->transparent = -1;
    
    if(loadheader(&screen, fp) == -1)
        goto error_exit;
	loadgce(fp, &gce);
	answer->transparent = gce.hastransparency ? gce.transparent : -1;
  //  answer->transparent = loadtransparency(fp, 0);
    if(screen.global_colourmap)
        loadpalette(fp, pal, 1 << screen.bits_per_pixel);
    answer->width = screen.screenwidth;
    answer->height = screen.screenheight;
    answer->background = screen.background;
    answer->store = 0;
    ...
    do {
        ...
        if(loadimageheader(&header, fp) == -1)
            goto error_exit;
        temp = realloc(answer->frames, (answer->Nframes+1)*sizeof(GIFFRAME));
        if(!temp)
            goto error_exit;
        answer->frames = temp;
        answer->Nframes++;
        if(header.use_local)
            loadpalette(fp, answer->frames[answer->Nframes-1].pal, 1 << header.bits_per_pixel);
        else
            memcpy(answer->frames[answer->Nframes-1].pal, pal, 256*3);
        answer->frames[answer->Nframes-1].image = malloc(header.width * header.height);
        if(!answer->frames[answer->Nframes-1].image)
            goto error_exit;
        if(loadraster(answer->frames[answer->Nframes-1].image, fp, header.width, header.height) == -1)
            goto error_exit;
        if(header.interlaced)
            if(uninterlace(answer->frames[answer->Nframes-1].image, header.width, header.height))
                goto error_exit;
        answer->frames[answer->Nframes-1].left = header.left;
        answer->frames[answer->Nframes-1].top = header.top;
        answer->frames[answer->Nframes-1].width = header.width;
        answer->frames[answer->Nframes-1].height = header.height;
		answer->frames[answer->Nframes-1].transparent = gce.hastransparency ? gce.transparent : -1;
        answer->frames[answer->Nframes-1].disposal = gce.disposal;
		answer->frames[answer->Nframes-1].delay = gce.delay;
...
unsigned char *gif_getfirstframe(GIF *gif, int *width, int *height, unsigned char *pal, int *transparent)
{
    ...
    answer = malloc(gif->width * gif->height);
    if(!answer)
        return 0;
    memset(answer, gif->background, gif->width * gif->height);
    dx = gif->frames[0].left;
    dy = gif->frames[0].top;
    for(y=0;y<gif->frames[0].height;y++)
        for(x=0;x<gif->frames[0].width;x++)
        {
            answer[(y+dy)*gif->width + x + dx] = gif->frames[0].image[y*gif->frames[0].width+x];
        }
    gif->current = 0;
    
    memcpy(pal, gif->frames[0].pal, 256*3);
...
int gif_getnextframe(GIF *gif, unsigned char *buff)
{
    ...
    if(gif->current == gif->Nframes -1)
        return 0;
    switch(gif->frames[gif->current].disposal)
    {
        case 0:
            break;
        case 1:
            break;
        case 2:
			if (gif->frames[gif->current].transparent != -1)
				bg = gif->frames[gif->current].transparent;
			else
				bg = gif->background;
            dx = gif->frames[gif->current].left;
            dy = gif->frames[gif->current].top;
            for(y=0;y<gif->frames[gif->current].height;y++)
                for(x=0;x<gif->frames[gif->current].width;x++)
                {
                    buff[(y+dy)*gif->width + x + dx] = bg;
                }
            break;
            
        case 3:
           if(gif->store)
               memcpy(buff, gif->store, gif->width * gif->height);
            break;
            
    }
    gif->current++;
    if(gif->frames[gif->current].disposal == 3)
    {
        if(!gif->store)
            gif->store = malloc(gif->width * gif->height);
        memcpy(gif->store, buff, gif->width * gif->height);
    }
    ...
    for(y=0;y<gif->frames[gif->current].height;y++)
        for(x=0;x<gif->frames[gif->current].width;x++)
        {
            if(  gif->frames[gif->current].image[y*gif->frames[gif->current].width+x]
               != gif->frames[gif->current].transparent)
                buff[(y+dy)*gif->width + x + dx] = gif->frames[gif->current].image[y*gif->frames[gif->current].width+x];
        }
