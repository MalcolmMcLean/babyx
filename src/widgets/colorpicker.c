...
BBX_RGBA pickcolor(BABYX *bbx, BBX_RGBA col)
{
  COLORPICKER cp;
  double hue;
  unsigned char s, v;
  ...
  rgb = (bbx_red(col) << 16) | (bbx_green(col) << 8) | bbx_blue(col);

  cp.bbx = bbx;
  cp.pan = bbx_dialogpanel(bbx, "Color Picker", 400, 300, layout, &cp);
  cp.can = bbx_canvas(bbx, cp.pan, 50, 50, col);
  cp.hue_can = bbx_canvas(bbx, cp.pan, 40, 256, col);
  cp.satval_can = bbx_canvas(bbx, cp.pan, 256, 256, col);
  ...
  bbx_dialogpanel_setclosefunc(cp.pan, presscancel, &cp);
  ...
  if (cp.ok)
  {
	  ...
	  return bbx_rgba( (rgb >> 16) & 0xFF, (rgb >> 8) & 0xFF, rgb & 0xFF, 0xFF);
  }
  else
	  return cp.oldcol;
}
